...
getQuantity3 = function(options, sideboard, DecksData_id){


    var total = 0;
    if(sideboard == true){
        DecksData.findOne({_id : DecksData_id}).sideboard.forEach(function(cardObj){
            total += cardObj.quantity;
        });
    }else{
        options = typeOptions[options];
        var cards = CardsData.find(options).map(function(p) { return p.name });
        var deck = DecksData.findOne({_id : DecksData_id});
        cards.forEach(function(cardsObj){
            var value = deck.main.find(function(obj2){
                return cardsObj ==  obj2.name;
            });
